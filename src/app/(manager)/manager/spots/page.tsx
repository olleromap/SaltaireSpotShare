...
import { FLOOR_LAYOUTS } from "@/lib/floor-layouts";
import FloorMap from "@/components/manager/FloorMap";
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          {spots.length} spots · {spots.filter((s) => s.ownerId).length} assigned
        </p>
      </div>
        ...
        {byFloor.map(({ floor, spots: floorSpots }) => {
          ...
          return (
            <Card key={floor}>
              ...
              <CardContent>
                {layout ? (
                  <FloorMap floor={floor} grid={layout} spots={floorSpots} />
                ) : (
                  <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
                    {floorSpots.map((spot) => (
                      <Link key={spot.id} href={`/manager/spots/${spot.id}`}>
                        <div
                          className={`aspect-square rounded-lg flex flex-col items-center justify-center text-xs font-medium cursor-pointer transition-all hover:scale-105 hover:shadow-md ${
                            spot.ownerId
                              ? "bg-[#1e4d8c]/10 text-[#1e4d8c] border border-[#1e4d8c]/20"
                              : "bg-slate-100 text-slate-400 border border-slate-200"
                          }`}
                        >
                          <span className="text-[10px] truncate px-1">{spot.spotNumber}</span>
                          {spot.owner && (
                            <span className="text-[9px] truncate px-1 opacity-70">
